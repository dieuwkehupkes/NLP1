...
def random_batch(batch_size, vocab_source, vocab_target, pairs, use_cuda):
    input_seqs = []
    target_seqs = []
    ...
    for i in range(batch_size):
        ...
        # input_seqs.append(vocab_source.indexes_from_sentence(pair[0]))
        # target_seqs.append(vocab_target.indexes_from_sentence(pair[1]))
    ...
    input_padded = [vocab_source.pad_seq(s, max(input_lengths)) for s in input_seqs]
    target_lengths = [len(s) for s in target_seqs]
    target_padded = [vocab_target.pad_seq(s, max(target_lengths)) for s in target_seqs]
